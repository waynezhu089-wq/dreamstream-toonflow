...
export type GateAdapter = (context: StageGateContext, q?: Knex.Transaction) => Promise<StageGateResult>;
...
export async function checkGate(key: string | null, context: StageGateContext, q?: Knex.Transaction): Promise<StageGateResult> {
  ...
  try {
    const result = await adapter(context, q);
...
registerGate("advertisement.asset-ready", async (context, q) => {
  const state = await readState(context.projectId, context.scriptId, q);
